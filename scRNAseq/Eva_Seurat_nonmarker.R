...
plt <- plot_grid(p1, p2)
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","UMAP",".pdf",sep=""),plot = plt,width = 4, height = 4)
dev.off()  
...
plt <- plot_grid(p1, p2)
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","TSNE_Split",".pdf",sep=""),plot = plt,width = 8, height = 4)
dev.off()

DimPlot(xenopus.combined, reduction = "umap", split.by = "stim")
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","UMAP_Split",".pdf",sep=""),width = 8, height = 4)
dev.off() 
...
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","TSNE_Split",".pdf",sep=""),width = 8, height = 4)
dev.off() 


#
#c("XAO0000074","XAO0005170","XAO0005169","XAO0000042","XAO0004091","XAO0000068","XAO0000090","XAO0000055","XAO0000083",
#"XAO0000084","XAO0000107","XAO0000049","XAO0004392","XAO0004085","XAO0004517","XAO0000249","XAO0000249",
#"XAO0000036","XAO0000305","XAO0003245","XAO0000028","XAO0000085","XAO0000320","XAO0000311",
#"XAO0003255","XAO0000259","XAO0000092","XAO0000295","XAO0003248","XAO0005045","XAO0004068","XAO0004067","XAO0004066")

#XAO0000074 involuting marginal zone
#XAO0005170 left-right organizer
#XAO0005169  left-right organizer primordium
#XAO0000042 neuroectoderm
#XAO0004091 non-neural extoderm
#XAO0000068 marginal zone
#XAO0000090 endoderm
#XAO0004513 foregut endoderm
#XAO0005247 hindgut endoderm,
#XAO0000055 notocord
#XAO0000083 involuted dorsal mesoderm
#XAO0000084 involuted ventral mesoderm
#XAO0000107  tail bid
#XAO0000049 cement gland primordium
#XAO0004392 goblet cell
#XAO0004085 inonocyte
#XAO0004517  ciliated epidermal progenitor
#XAO0000249 neural plat
#XAO0000249 neural plate border

#XAO0000036 animal cap
#XAO_0000305 cranial placode
#XAO_0003245  ectodermal cell
#XAO_0000028 epidermis
#XAO_0000320 sensoril layer
#XAO_0000085 intermediate mesoderm
#XAO_0000311 lateral plate mesoderm
#XAO_0003255 mesodermal cell
#XAO_0000259 paraxial mesoderm
#XAO_0000092  archenteron
#XAO_0000295 central endoderm
#XAO_0005045 dorsal crest of the endoderm
#XAO_0003248 endodermal cell
#XAO0004513
#XAO_0004068 sub-blastoporal endoderm
#XAO_0004067 suprablastoporal endoderm
#XAO_0004066 vegetal endoderm
#XAO1000023 St13
#XAO1000017 St7
#XAO1000031 St10.5
#XAO1000033 12.5

labl<-c("involuting marginal zone","left-right organizer","left-right organizer primordium","neuroectoderm",
"non-neural extoderm","marginal zone","notocord","involuted dorsal mesoderm","involuted ventral mesoderm",
"tail bid","cement gland primordium","goblet cell","inonocyte","ciliated epidermal progenitor","neural plate",
"animal cap","cranial placode","ectodermal cell","epidermis","intermediate mesoderm","sensoril layer","lateral plate mesoderm",
"mesodermal cell","paraxial mesoderm","archenteron","central endoderm","endodermal cell","dorsal crest of the endoderm",
"sub-blastoporal endoderm","suprablastoporal endoderm","vegetal endoderm","St13","St7","St10.5","St12.5")

  
early <- c("XAO0000074","XAO0005170","XAO0005169","XAO0000042","XAO0004091","XAO0000068","XAO0000090","XAO0000055","XAO0000083",
  "XAO0000084","XAO0000107","XAO0000049","XAO0004392","XAO0004085","XAO0004517","XAO0000249",
  "XAO0000036","XAO0000305","XAO0003245","XAO0000028","XAO0000085","XAO0000320","XAO0000311",
  "XAO0003255","XAO0000259","XAO0000092","XAO0000295","XAO0003248","XAO0005045","XAO0004068","XAO0004067","XAO0004066",
  "XAO1000023","XAO1000017","XAO1000031","XAO1000033")


#early = c("sub-blastoporal endoderm","suprablastoporal endoderm","vegetal endoderm","dorsal crest of the endoderm","endodermal cell","archenteron","central endoderm","somite","paraxial mesoderm","mesodermal cell","lateral plate mesoderm","intermediate mesoderm","sensoril layer","animal cap","ectodermal cell","cranial placode","neuroectoderm","non-neural ectoderm","marginal zone","endoderm","foregut","hindgut","organizer","notochord","involuted ventral mesoderm","involuted dorsal mesoderm","tail bud","cement gland primordium","goblet cell","ionocyte","ciliated epidermal progenitor","neural plate","neural plate border")
OnMem <- read.delim("/Users/christopherpenfold/Desktop/Eva/Motifs/Genelists_from_Eva_April2018/EndoNT_Filter_DonorsvsIVFdownFDR_DonsRPKM1_IVFvsNT_FDRup__OnMemFDR.txt")
OffMem <- read.delim("/Users/christopherpenfold/Desktop/Eva/Motifs/Genelists_from_Eva_April2018/EndoNT_Filter_DonorsvsIVFupFDR_IVFvsNT_FDRdown__OffMemFDR.txt")
tau <- 1
mat1 <- as.matrix(GetAssayData(SIGAH12, slot = "data"))
#mat1 <- as.matrix(GetAssayData(xenopus.combined, slot = "data"))
#mat1 <- mat02[,1:dim(mat01)[2]]
#mat2 <- mat02[,(dim(mat01)[2]+1):dim(mat02)[2]]
...
for (j in 1:length(early)){
  mg <- Markers$V2[grep(as.character(early[j]),as.character(Markers$V5))]
    for (i in 1:dim(mat1)[2]){
      N_1[i,j] <- length(intersect(rownames(mat1)[which(mat1[,i]>tau)],mg))
      N_2[i,j] <- length((rownames(mat1)[which(mat1[,i]>tau)]))      
...
for (i in 1:dim(mat1)[2]){
N_1[i,j+1] <- length(intersect(rownames(mat1)[which(mat1[,i]>tau)],OnMem[,1]))
N_2[i,j+1] <- length((rownames(mat1)[which(mat1[,i]>tau)]))      
ps <- phyper(N_1[i,j+1]-1, N_2[i,j+1], dim(mat1)[1]-N_2[i,j+1], dim(OnMem)[1], lower.tail= FALSE)
N_3[i,j+1] <- ps
}

for (i in 1:dim(mat1)[2]){
  N_1[i,j+2] <- length(intersect(rownames(mat1)[which(mat1[,i]>tau)],OffMem[,1]))
  N_2[i,j+2] <- length((rownames(mat1)[which(mat1[,i]>tau)]))      
  ps <- phyper(N_1[i,j+2]-1, N_2[i,j+2], dim(mat1)[1]-N_2[i,j+2], dim(OffMem)[1], lower.tail= FALSE)
  N_3[i,j+2] <- ps
}

mat2 <- as.matrix(GetAssayData(SIGAH5, slot = "data"))
...
for (j in 1:length(early)){
  ...
  for (i in 1:dim(mat2)[2]){
    M_1[i,j] <- length(intersect(rownames(mat2)[which(mat2[,i]>tau)],mg))
    M_2[i,j] <- length((rownames(mat2)[which(mat2[,i]>tau)]))      
    ps <- phyper(M_1[i,j]-1, M_2[i,j], dim(mat2)[1]-M_2[i,j], length(mg), lower.tail= FALSE)
  }
    M_3[i,j] <- ps
}
for (i in 1:dim(mat1)[2]){
  M_1[i,j+1] <- length(intersect(rownames(mat2)[which(mat2[,i]>tau)],OnMem[,1]))
  M_2[i,j+1] <- length((rownames(mat2)[which(mat2[,i]>tau)]))      
  ps <- phyper(M_1[i,j+1]-1, M_2[i,j+1], dim(mat2)[1]-M_2[i,j+1], dim(OnMem)[1], lower.tail= FALSE)
  M_3[i,j+1] <- ps
}
for (i in 1:dim(mat1)[2]){
  M_1[i,j+2] <- length(intersect(rownames(mat2)[which(mat2[,i]>tau)],OffMem[,1]))
  M_2[i,j+2] <- length((rownames(mat2)[which(mat2[,i]>tau)]))      
...
No <- c(N_1[,j],M_1[,j]) #Concatenate the counts of genes
pvalss <- c(N_3[,j],M_3[,j]) #Concatenate the counts of genes
padj <- p.adjust(pvalss, method = "bonferroni")

No[which(No<mean(No))]<-0
pvalss <- 1- pvalss
padj <- 1- padj

vec2add <- No #padj-mean(padj) #c(N_3[,j],M_3[,j]) #as.data.frame(t(c(N_3[,j],M_3[,j])))
names(vec2add) <- colnames(x = xenopus.combined)
xenopus.combined <- AddMetaData(object = xenopus.combined,metadata = vec2add,col.name = early[j])

vec2add <- pvalss #padj-mean(padj) #c(N_3[,j],M_3[,j]) #as.data.frame(t(c(N_3[,j],M_3[,j])))
names(vec2add) <- colnames(x = xenopus.combined)
xenopus.combined <- AddMetaData(object = xenopus.combined,metadata = vec2add,col.name = paste(early[j],'pval') )

vec2add <- padj #padj-mean(padj) #c(N_3[,j],M_3[,j]) #as.data.frame(t(c(N_3[,j],M_3[,j])))
names(vec2add) <- colnames(x = xenopus.combined)
xenopus.combined <- AddMetaData(object = xenopus.combined,metadata = vec2add,col.name = paste(early[j],'adjpval') )

FeaturePlot(xenopus.combined, features = early[j], cols = c("grey", "red"))
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/",early[j],"_",tau,"_ns.pdf",sep=""),width = 4, height = 4)
FeaturePlot(xenopus.combined, features = early[j], split.by = "stim", cols = c("grey", "red"))
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/",early[j],"_",tau,"_nst.pdf",sep=""),width = 8, height = 4)

FeaturePlot(xenopus.combined, features = paste(early[j],'pval'), cols = c("grey", "red"))
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/",early[j],"_",tau,"_prob.pdf",sep=""),width = 4, height = 4)
FeaturePlot(xenopus.combined, features = paste(early[j],'pval'), split.by = "stim", cols = c("grey", "red"))
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/",early[j],"_",tau,"_probt.pdf",sep=""),width = 8, height = 4)

FeaturePlot(xenopus.combined, features = paste(early[j],'adjpval'), cols = c("grey", "red"))
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/",early[j],tau,"_adjprob.pdf",sep=""),width = 4, height = 4)
FeaturePlot(xenopus.combined, features = paste(early[j],'adjpval'), split.by = "stim", cols = c("grey", "red"))
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/",early[j],tau,"_adjprobt.pdf",sep=""),width = 8, height = 4)
...
M_1 <- matrix(0, nrow = length(early)+2,ncol=10)
M_2 <- matrix(0, nrow = length(early)+2,ncol=10)
M_3 <- matrix(0, nrow = length(early)+2,ncol=10)

for (i in 1:(length(early))){
  
  mg <- Markers$V2[grep(as.character(early[i]),as.character(Markers$V4))]
  M_1[i,1] <- length(intersect(mg,rownames(cl0.markers)))
  M_2[i,1] <- length(rownames(cl0.markers))      
  ps <- phyper(M_1[i,1]-1, M_2[i,1], dim(mat2)[1]-M_2[i,1], length(mg), lower.tail= FALSE)
  M_3[i,1] <- ps
  
  M_1[i,2] <- length(intersect(mg,rownames(cl1.markers)))
  M_2[i,2] <- length(rownames(cl1.markers))      
  ps <- phyper(M_1[i,2]-1, M_2[i,2], dim(mat2)[1]-M_2[i,2], length(mg), lower.tail= FALSE)
  M_3[i,2] <- ps  

  M_1[i,3] <- length(intersect(mg,rownames(cl2.markers)))
  M_2[i,3] <- length(rownames(cl2.markers))      
  ps <- phyper(M_1[i,3]-1, M_2[i,3], dim(mat2)[1]-M_2[i,3], length(mg), lower.tail= FALSE)
  M_3[i,3] <- ps  
  
  M_1[i,4] <- length(intersect(mg,rownames(cl3.markers)))
  M_2[i,4] <- length(rownames(cl3.markers))      
  ps <- phyper(M_1[i,4]-1, M_2[i,4], dim(mat2)[1]-M_2[i,4], length(mg), lower.tail= FALSE)
  M_3[i,4] <- ps  
  
  
  M_1[i,5] <- length(intersect(mg,rownames(cl4.markers)))
  M_2[i,5] <- length(rownames(cl4.markers))      
  ps <- phyper(M_1[i,5]-1, M_2[i,5], dim(mat2)[1]-M_2[i,5], length(mg), lower.tail= FALSE)
  M_3[i,5] <- ps  
  
  
  M_1[i,6] <- length(intersect(mg,rownames(cl5.markers)))
  M_2[i,6] <- length(rownames(cl5.markers))      
  ps <- phyper(M_1[i,6]-1, M_2[i,6], dim(mat2)[1]-M_2[i,6], length(mg), lower.tail= FALSE)
  M_3[i,6] <- ps  
  
  M_1[i,7] <- length(intersect(mg,rownames(cl6.markers)))
  M_2[i,7] <- length(rownames(cl6.markers))      
  ps <- phyper(M_1[i,7]-1, M_2[i,7], dim(mat2)[1]-M_2[i,7], length(mg), lower.tail= FALSE)
  M_3[i,7] <- ps  
  
  M_1[i,8] <- length(intersect(mg,rownames(cl7.markers)))
  M_2[i,8] <- length(rownames(cl7.markers))      
  ps <- phyper(M_1[i,8]-1, M_2[i,8], dim(mat2)[1]-M_2[i,8], length(mg), lower.tail= FALSE)
  M_3[i,8] <- ps  
    
  M_1[i,9] <- length(intersect(mg,rownames(cl8.markers)))
  M_2[i,9] <- length(rownames(cl8.markers))      
  ps <- phyper(M_1[i,9]-1, M_2[i,9], dim(mat2)[1]-M_2[i,9], length(mg), lower.tail= FALSE)
  M_3[i,9] <- ps  
  
  M_1[i,10] <- length(intersect(mg,rownames(cl9.markers)))
  M_2[i,10] <- length(rownames(cl9.markers))      
  ps <- phyper(M_1[i,10]-1, M_2[i,10], dim(mat2)[1]-M_2[i,10], length(mg), lower.tail= FALSE)
  M_3[i,10] <- ps  
}
...
for (i in 1:length(early)){
  plots <- VlnPlot(xenopus.combined, features = early[i], split.by = "stim", group.by = "celltype", pt.size = 0, combine = FALSE)
  CombinePlots(plots = plots, ncol = 1)
  ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","EarlyMarkers_",i,"_violin.pdf",sep=""),width = 8, height = 8)
}
