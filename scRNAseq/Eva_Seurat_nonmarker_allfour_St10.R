...
SIGAA2$stim <- "SIGAA2"
SIGAA2$isnt <- "IVF2"
SIGAA2 <- subset(SIGAA2, subset = nFeature_RNA > 500)
...
SIGAB2 <- subset(SIGAB2, subset = nFeature_RNA > 500)
...
xenopus.anchors <- FindIntegrationAnchors(object.list = list(SIGAH5, SIGAA2, SIGAH12, SIGAB2), dims = 1:20, anchor.features = 2000)
...
plt <- plot_grid(p1, p2)
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","UMAP",".pdf",sep=""), plot = plt,width = 8, height = 4)
...
plt <- plot_grid(p1, p2)
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","TSNE",".pdf",sep=""),plot = plt,width = 16, height = 4)
dev.off()

DimPlot(xenopus.combined, reduction = "umap", split.by = "stim")
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","UMAP_Split",".pdf",sep=""),width = 16, height = 4)
dev.off() 

DimPlot(xenopus.combined, reduction = "tsne", split.by = "stim")
ggsave(filename=paste("~/Desktop/Eva/OnClusterPlots/","TSNE_Split",".pdf",sep=""),width = 16, height = 4)
dev.off() 
