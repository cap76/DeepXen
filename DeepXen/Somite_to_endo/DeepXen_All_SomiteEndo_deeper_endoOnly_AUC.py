...
model = load_model('/mnt/scratch/gurdon/cap76/DeepXen/ResultsSomiteEndoAll/Model_deeper_endoOnly.h5')
...
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsSomiteEndoAll/PredictionScores_endoOnly.csv')
...
np.save('ResultsSomiteEndoAll/fpr_kearst_endoOnly.npy', fpr_keras)
np.save('ResultsSomiteEndoAll/tpr_kearst_endoOnly.npy', tpr_keras)
np.save('ResultsSomiteEndoAll/pr_kearst_endoOnly.npy', pr_keras)
np.save('ResultsSomiteEndoAll/re_kearst_endoOnly.npy', re_keras)
plt.figure()
plt.plot(re_kerast, pr_kerast, label='Combined') #, label='ROC curve (area = %0.2f)' % roc_auc[i])
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsSomiteEndoAll/PR_endoOnly.pdf')
plt.draw()
plt.figure()
...
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsSomiteEndoAll/AUC_endoOnly.pdf')
plt.draw()
...
M[0,24]= sum(OtherP)
M[0,25]= sum(OtherT)
pd.DataFrame(M).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsSomiteEndoAll/Counts_endoOnly.csv')
