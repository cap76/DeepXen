...
Output2 = genfromtxt('BWESCiPSC/EndoNT_On_all.inde.bed',delimiter='\t',dtype=None)

#Load in the list of genes
onlist = genfromtxt('RNAseq/ON_HDF_IVF_iPSC_ESC.csv',delimiter='\t',dtype=None)
downlist = genfromtxt('RNAseq/RD_HDF_IVF_iPSC_ESC.csv',delimiter='\t',dtype=None)
offlist = genfromtxt('RNAseq/OFF_HDF_IVF_iPSC_ESC.csv',delimiter='\t',dtype=None)
uplist = genfromtxt('RNAseq/RU_HDF_IVF_iPSC_ESC.csv',delimiter='\t',dtype=None)
complist = genfromtxt('ESCiPSC/ComplementarySetGenes.txt',delimiter='\t',dtype=None)
...
for i in range(0, np.shape(Yalt)[0]):
   ...
   x2 = (explist['f20'][arr_ind].astype('f')+explist['f21'][arr_ind].astype('f')+explist['f26'][arr_ind].astype('f')+explist['f27'][arr_ind].astype('f'))/4
...
fil=sorted(glob.glob('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWESCiPSC/*.tab'))
...
model.save('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWESCiPSC/Model_deeper.h5')
...
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWESCiPSC/prediction_scores_deeper.csv')
...
np.save('BWESC/fpr_ESC_', fpr_keras)
np.save('BWESC/tpr_ESC_', tpr_keras)
np.save('BWESC/pr_ESC_', pr_keras)
np.save('BWESC/re_ESC_', re_keras)
...
pd.DataFrame(prediction, columns=['Chr','start','end','Gene','IVF','Donor','NT','FC','pVal','FC','pVal','ON','RepDown','Off','RepUp','Neg','pC1','pC2','pC3','pC4','pC5']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWECiPSC/prediction_deeper.csv')
