...
Output2 = genfromtxt('BWEC/EndoNT_On_all.inde.bed',delimiter='\t',dtype=None)

#Load in the list of genes
onlist = genfromtxt('RNAseq/ON_HDF_IVF_NT_EC.csv',delimiter='\t',dtype=None)
downlist = genfromtxt('RNAseq/RD_HDF_IVF_NT_EC.csv',delimiter='\t',dtype=None)
offlist = genfromtxt('RNAseq/OFF_HDF_IVF_NT_EC.csv',delimiter='\t',dtype=None)
uplist = genfromtxt('RNAseq/RU_HDF_IVF_NT_EC.csv',delimiter='\t',dtype=None)
complist = genfromtxt('EC/ComplementarySetGenes.txt',delimiter='\t',dtype=None)
...
explist = genfromtxt('FPKM_ext.csv',delimiter='\t',dtype=None, skip_header=1)

for i in range(0, np.shape(Yalt)[0]):
   arr_ind = np.where(Output2['f3'][i] == explist['f0'])
   #arr_ind = np.where(Output2['f3'][i] == explist[:,0])
   ...
   #x1 = (explist[arr_ind,1].astype('f')+explist[arr_ind,2].astype('f')+explist[arr_ind,3].astype('f'))/3
   #x2 = (explist[arr_ind,6].astype('f')+explist[arr_ind,7].astype('f')+explist[arr_ind,12].astype('f')+explist[arr_ind,13].astype('f'))/4
...
model.save('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWEC/Model_deeper.h5')
...
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWEC/prediction_scores_deeper.csv')
...
np.save('BWEC/fpr_EC_', fpr_keras)
np.save('BWEC/tpr_EC_', tpr_keras)
np.save('BWEC/pr_EC_', pr_keras)
np.save('BWEC/re_EC_', re_keras)
...
pd.DataFrame(prediction, columns=['Chr','start','end','Gene','IVF','Donor','NT','FC','pVal','FC','pVal','ON','RepDown','Off','RepUp','Neg','pC1','pC2','pC3','pC4','pC5']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWEC/prediction_deeper.csv')
