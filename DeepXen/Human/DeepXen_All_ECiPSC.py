...
Output2 = genfromtxt('BWECiPSC/EndoNT_On_all.inde.bed',delimiter='\t',dtype=None)

#Load in the list of genes
onlist = genfromtxt('RNAseq/ON_HDF_IVF_iPSC_EC.csv',delimiter='\t',dtype=None)
downlist = genfromtxt('RNAseq/RD_HDF_IVF_iPSC_EC.csv',delimiter='\t',dtype=None)
offlist = genfromtxt('RNAseq/OFF_HDF_IVF_iPSC_EC.csv',delimiter='\t',dtype=None)
uplist = genfromtxt('RNAseq/RU_HDF_IVF_iPSC_EC.csv',delimiter='\t',dtype=None)
complist = genfromtxt('ECiPSC/ComplementarySetGenes.txt',delimiter='\t',dtype=None)
...
for i in range(0, np.shape(Yalt)[0]):
   ...
   x2 = (explist['f18'][arr_ind].astype('f')+explist['f19'][arr_ind].astype('f')+explist['f24'][arr_ind].astype('f')+explist['f25'][arr_ind].astype('f'))/4
...
fil=sorted(glob.glob('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWECiPSC/*.tab'))
...
model.save('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWECiPSC/Model_deeper.h5')
...
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWECiPSC/prediction_scores_deeper.csv')
