...
Output2 = genfromtxt('BWCMiPSC/EndoNT_On_all.inde.bed',delimiter='\t',dtype=None)

#Load in the list of genes
onlist = genfromtxt('RNAseq/ON_HDF_IVF_iPSC_CM.csv',delimiter='\t',dtype=None)
downlist = genfromtxt('RNAseq/RD_HDF_IVF_iPSC_CM.csv',delimiter='\t',dtype=None)
offlist = genfromtxt('RNAseq/OFF_HDF_IVF_iPSC_CM.csv',delimiter='\t',dtype=None)
uplist = genfromtxt('RNAseq/RU_HDF_IVF_iPSC_CM.csv',delimiter='\t',dtype=None)
complist = genfromtxt('CMiPSC/ComplementarySetGenes.txt',delimiter='\t',dtype=None)
...
for i in range(0, np.shape(Yalt)[0]):
   ...
   x2 = (explist['f16'][arr_ind].astype('f')+explist['f17'][arr_ind].astype('f')+explist['f22'][arr_ind].astype('f')+explist['f23'][arr_ind].astype('f'))/4
...
fil=sorted(glob.glob('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWCMiPSC/*.tab'))
...
model.save('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWCMiPSC/Model_deeper.h5')
...
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWCMiPSC/prediction_scores_deeper.csv')
...
np.save('BWCM/fpr_CM_', fpr_keras)
np.save('BWCM/tpr_CM_', tpr_keras)
np.save('BWCM/pr_CM_', pr_keras)
np.save('BWCM/re_CM_', re_keras)
...
pd.DataFrame(prediction, columns=['Chr','start','end','Gene','IVF','Donor','NT','FC','pVal','FC','pVal','ON','RepDown','Off','RepUp','Neg','pC1','pC2','pC3','pC4','pC5']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/Human/BWCMiPSC/prediction_deeper.csv')
