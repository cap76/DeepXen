...
Output2 = genfromtxt('/mnt/scratch/gurdon/cap76/DeepXen/BW_SomiteEcto_All_GB/allpeaks_labelled_cum_final.s.bed',delimiter='\t',dtype=None)

#Load in the list of genes
onlist = genfromtxt('ChIP_SomiteEcto_All_GB/OnMemFDRtGenes.txt',delimiter='\t',dtype=None)
downlist = genfromtxt('ChIP_SomiteEcto_All_GB//ReprogrammedDowntGenes.txt',delimiter='\t',dtype=None)
offlist = genfromtxt('ChIP_SomiteEcto_All_GB/OffMemFDRtGenes.txt',delimiter='\t',dtype=None)
uplist = genfromtxt('ChIP_SomiteEcto_All_GB/ReprogrammedUptGenes.txt',delimiter='\t',dtype=None)
complist = genfromtxt('ChIP_SomiteEcto_All_GB/ComplementarySettGenes.txt',delimiter='\t',dtype=None)
...
#explist = genfromtxt('ChIP_SomiteEcto_All_GB//edger_de_pairing_BIGtable_endoIVF_ectoDonor_endoNT_plus_DE.p.csv',delimiter='\t',dtype=None)
explist = genfromtxt('ChIP_SomiteEcto_All_GB/edger_de_pairing_BIGtable_ectoIVF_somiteDonor_ectoNT_plus_DE.p.csv',delimiter='\t',dtype=None)
...
fil=sorted(glob.glob('/mnt/scratch/gurdon/cap76/DeepXen/BW_SomiteEcto_All_GB/*.tab'))
...
#visible1 = Input(shape=(300,np.shape(fil)[0]))
...
visible2 = Input(shape=(1,np.shape(Xexp)[2]))
...
predictions = model.predict([Xexp[:,:]], batch_size=1000)
...
Scores[2,0] = model.evaluate([Xexp[valset,:,:]], Yalt[valset,:], batch_size=32)[1]
 									             #ResultsSomiteEctoAllGB
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsSomiteEctoAllGB/prediction_scores_pos.csv')

model.save('/mnt/scratch/gurdon/cap76/DeepXen/ResultsSomiteEctoAllGB/Model_pos.h5')
...
#        Xtemp1[0:np.shape(Xtemp1)[0],0:300,j] = (0 -np.nanmean(Xchra[:,0:300,j])) / np.nanstd(Xchra[:,0:300,j])
...
#                        Xtemp1[0:np.shape(Xtemp1)[0],0:300,ind1] = (0 -np.nanmean(Xchra[:,0:300,ind1])) / np.nanstd(Xchra[:,0:300,ind1])
#                np.random.seed(j)
#                Xtemp1[0:np.shape(Xtemp1)[0],0:300,j] = (0 -np.nanmean(Xchra[:,0:300,j])) / np.nanstd(Xchra[:,0:300,j])
#                score = model.evaluate([Xtemp1,Xexp[:,:,:]], Yalt, batch_size=32)
#                XP[i,j] = score[1]
#
#pd.DataFrame(XP, columns=fil).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsSomiteEctoAllGB/prediction_settozero.csv')
...
pd.DataFrame(prediction, columns=['Chr','start','end','Gene','IVF','Donor','NT','FC','pVal','FC','pVal','ON','RepDown','Off','RepUp','Neg','pC1','pC2','pC3','pC4','pC5']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsSomiteEctoAll/predictionGB_pos.csv')

#Now process the results
#os.chdir('ResultsEndoAllGB')
#os.system('./TPall2.sh')
...
OutSomEcto = genfromtxt('BW_SomiteEcto_All_GB/allpeaks_labelled_cum_final.s.bed',delimiter='\t',dtype=None)
...
XchrSomEcto = np.zeros((np.shape(OutSomEcto)[0],300,np.shape(fil)[0]))
for k in range(0, np.shape(fil)[0]):
...
for k in range(0, np.shape(fil)[0]):
      XchrSomEcto[:,0:300,k] = ( XchrSomEcto[:,0:300,k] - np.nanmean(Xchra[:,0:300,k]) ) / np.nanstd(Xchra[:,0:300,k])
...
ScoresSomEct[0,0] = model.evaluate([XchrSomEcto,Xexp2], YSomEcto, batch_size=32)[1]
pd.DataFrame(ScoresSomEct, columns=['Accuracy']).to_csv('ResultsEctoAllGB/prediction_SomEct_scores_pos.csv')

predictions = model.predict([XchrSomEcto,Xexp2], batch_size=1000)
...
df12 = pd.DataFrame(YSomEcto[i,0],columns=['C8'])
df13 = pd.DataFrame(YSomEcto[i,1],columns=['C9'])
df14 = pd.DataFrame(YSomEcto[i,2],columns=['C10'])
df15 = pd.DataFrame(YSomEcto[i,3],columns=['C11'])
df16 = pd.DataFrame(YSomEcto[i,4],columns=['C12'])


df14b = pd.DataFrame(predictions,columns=['pC1','pC2','pC3','pC4','pC5'])

prediction = np.concatenate((df1, df2, df3, df4, df5, df6, df7, df8, df9, df10, df11, df12, df13, df14, df15, df16, df14b),1)
pd.DataFrame(prediction, columns=['Chr','start','end','Gene','IVF','Donor','NT','FC','pVal','FC','pVal','ON','RepDown','Off','RepUp','Neg','pC1','pC2','pC3','pC4','pC5']).to_csv('ResultsEctoAllGB/prediction_SomEcto_pos.csv')
