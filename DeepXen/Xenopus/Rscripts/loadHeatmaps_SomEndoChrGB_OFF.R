...
mat <- np$load("SomEndoFullGB/Up_attributions_OffTP.npy")
mat1 <- np$load("SomEndoFullGB/Off_attributions_OffTP.npy")
mat2 <- np$load("SomEndoFullGB/Off_attributions_OffTP_perm.npy")

#mat1 <- np$load("EndoFull/Down_attributions_OnTP_1.npy")


#Index for TP ON memory genes (to get get IDS etc)
offTP <- np$load("SomEndoFullGB/OffTP.npy")
...
pheatmap(t(SummaryMat),color =  redblue1(20), gaps_col = c(NCl,NCl*2),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster summary"),filename = "SomEndoFullGB/OFFRUPCl.pdf")

colnames(SummaryMat1) <- labs
rownames(SummaryMat1) <- paste("Cl",rep(1:NCl, each=1))
pheatmap(t(SummaryMat1),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=TRUE, main = c("Cluster summary"),filename = "SomEndoFullGB/OFFCl.pdf")
...
Msp <- uniqueMat1[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("SomEndoFullGB/OFFCl", i, ".pdf", sep="") )

Msp <- uniqueMat2[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("SomEndoFullGB/RUCl", i, ".pdf", sep="") )

Msp <- uniqueMat1[i,,]-uniqueMat2[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("SomEndoFullGB/OffClminusRU", i, ".pdf", sep="") )

Msp <- uniqueMat1[i,,]-uniqueMat3[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("SomEndoFullGB/OFFClminusPerm", i, ".pdf", sep="") )
