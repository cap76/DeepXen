...
mat <- np$load("EndoFullGB/Up_attributions_OffTP.npy")
mat1 <- np$load("EndoFullGB/Off_attributions_OffTP.npy")
mat2 <- np$load("EndoFullGB/Off_attributions_OffTP_perm.npy")

#mat1 <- np$load("EndoFull/Down_attributions_OnTP_1.npy")


#Index for TP ON memory genes (to get get IDS etc)
offTP <- np$load("EndoFullGB/OffTP.npy")

#Get the gene IDS etc
Data <-read.table("EndoFullGB/allpeaks_labelled_cum_final.se.bed",sep="\t", header = F, row.names=NULL)
genes <- Data$V4[offTP]
...
pheatmap(t(SummaryMat),color =  redblue1(20), gaps_col = c(NCl,NCl*2),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster summary"),filename = "EndoFullGB/OFFRUPCl.pdf")

colnames(SummaryMat1) <- labs
rownames(SummaryMat1) <- paste("Cl",rep(1:NCl, each=1))
pheatmap(t(SummaryMat1),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=TRUE, main = c("Cluster summary"),filename = "EndoFullGB/OFFCl.pdf")
...
Msp <- uniqueMat1[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/OFFCl", i, ".pdf", sep="") )

Msp <- uniqueMat2[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/RUCl", i, ".pdf", sep="") )

Msp <- uniqueMat1[i,,]-uniqueMat2[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/OffClminusRU", i, ".pdf", sep="") )

Msp <- uniqueMat1[i,,]-uniqueMat3[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/OFFClminusPerm", i, ".pdf", sep="") )
