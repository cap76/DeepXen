...
mat <- np$load("EndoFullGB/Down_attributions_OnTP.npy")
mat1 <- np$load("EndoFullGB/On_attributions_OnTP.npy")
mat2 <- np$load("EndoFullGB/On_attributions_OnTP_perm.npy")

#mat1 <- np$load("EndoFull/Down_attributions_OnTP_1.npy")


#Index for TP ON memory genes (to get get IDS etc)
onTP <- np$load("EndoFullGB/OnTP.npy")
...
pheatmap(t(SummaryMat),color =  redblue1(20), gaps_col = c(NCl,NCl*2),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster summary"),filename = "EndoFullGB/ONRDPCl.pdf")

colnames(SummaryMat1) <- labs
rownames(SummaryMat1) <- paste("Cl",rep(1:NCl, each=1))
pheatmap(t(SummaryMat1),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=TRUE, main = c("Cluster summary"),filename = "EndoFullGB/ONCl.pdf")
...
Msp <- uniqueMat1[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/ONCl", i, ".pdf", sep="") )

Msp <- uniqueMat2[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/RDCl", i, ".pdf", sep="") )

Msp <- uniqueMat1[i,,]-uniqueMat2[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/ONClminusRD", i, ".pdf", sep="") )

Msp <- uniqueMat1[i,,]-uniqueMat3[i,,]
colnames(Msp) <- labs
pheatmap(t(Msp),color =  redblue1(20),  gaps_row=c(10,20), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE, main = c("Cluster"),filename = paste("EndoFullGB/ONClminusPerm", i, ".pdf", sep="") )
