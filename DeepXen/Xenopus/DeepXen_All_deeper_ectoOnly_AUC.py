...
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/PredictionScores_ectoOnly.csv')
...
np.save('ResultsEndoAll/fpr_kearst_ectoOnly.npy', fpr_keras)
np.save('ResultsEndoAll/tpr_kearst_ectoOnly.npy', tpr_keras)
np.save('ResultsEndoAll/pr_kearst_ectoOnly.npy', pr_keras)
np.save('ResultsEndoAll/re_kearst_ectoOnly.npy', re_keras)
plt.figure()
plt.plot(re_kerast, pr_kerast, label='Combined') #, label='ROC curve (area = %0.2f)' % roc_auc[i])
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsEndoAll/PR_ectoOnly.pdf')
plt.draw()
plt.figure()
...
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsEndoAll/AUC_ectoOnly.pdf')
plt.draw()
...
M[0,24]= sum(OtherP)
M[0,25]= sum(OtherT)
pd.DataFrame(M).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/Counts_ectoOnly.csv')
