...
model = load_model('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/Model_deeper_neg_endoOnly.h5')

#model = load_model('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/Model_deeper_ectoOnly.h5')
plt.switch_backend('agg')
Scores = np.zeros((1,1))
Scores[0,0] = model.evaluate([Xchr[valset,:,:]], Yalt[valset,:], batch_size=32)[1]
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/PredictionScores_neg_endoOnly.csv')
classpred1 = model.predict([Xchr], batch_size=1000)
import matplotlib.pyplot as plt
from matplotlib import colors as mcolors
plt.switch_backend('agg')
from sklearn.metrics import roc_curve, precision_recall_curve,auc
fpr_keras, tpr_keras, thresholds_keras = roc_curve(Yalt.ravel(), model.predict([Xchr], batch_size=1000).ravel())
pr_keras, re_keras, thresholds_keras = precision_recall_curve(Yalt.ravel(), model.predict([Xchr], batch_size=1000).ravel())
fpr_kerast, tpr_kerast, thresholds_kerast = roc_curve(Yalt[valset,:].ravel(), model.predict([Xchr[valset,:,:]], batch_size=1000).ravel())
pr_kerast, re_kerast, thresholds_keras = precision_recall_curve(Yalt[valset,:].ravel(), model.predict([Xchr[valset,:,:]], batch_size=1000).ravel())
np.save('ResultsEndoAll/fpr_kearst_neg_endoOnly.npy', fpr_keras)
np.save('ResultsEndoAll/tpr_kearst_neg_endoOnly.npy', tpr_keras)
np.save('ResultsEndoAll/pr_kearst_neg_endoOnly.npy', pr_keras)
np.save('ResultsEndoAll/re_kearst_neg_endoOnly.npy', re_keras)
plt.figure()
plt.plot(re_kerast, pr_kerast, label='Combined') #, label='ROC curve (area = %0.2f)' % roc_auc[i])
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsEndoAll/PR_neg_endoOnly.pdf')
plt.draw()
plt.figure()
...
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsEndoAll/AUC_neg_endoOnly.pdf')
plt.draw()
...
M[0,24]= sum(OtherP)
M[0,25]= sum(OtherT)
pd.DataFrame(M).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/Counts_neg_endoOnly.csv')
