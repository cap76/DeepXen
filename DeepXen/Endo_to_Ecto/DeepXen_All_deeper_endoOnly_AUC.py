...
model = load_model('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/Model_deeper_endoOnly.h5')

plt.switch_backend('agg')
Scores = np.zeros((1,1))
Scores[0,0] = model.evaluate([Xchr[valset,:,:],Xexp[valset,:,:]], Yalt[valset,:], batch_size=32)[1]
pd.DataFrame(Scores, columns=['Accuracy']).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/PredictionScores_endoOnly.csv')
...
np.save('ResultsEndoAll/fpr_kearst_endoOnly.npy', fpr_keras)
np.save('ResultsEndoAll/tpr_kearst_endoOnly.npy', tpr_keras)
np.save('ResultsEndoAll/pr_kearst_endoOnly.npy', pr_keras)
np.save('ResultsEndoAll/re_kearst_endoOnly.npy', re_keras)
plt.figure()
plt.plot(re_kerast, pr_kerast, label='Combined') #, label='ROC curve (area = %0.2f)' % roc_auc[i])
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsEndoAll/PR_endoOnly.pdf')
plt.draw()
plt.figure()
...
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")
plt.savefig('ResultsEndoAll/AUC_endoOnly.pdf')
plt.draw()
...
M[0,24]= sum(OtherP)
M[0,25]= sum(OtherT)
pd.DataFrame(M).to_csv('/mnt/scratch/gurdon/cap76/DeepXen/ResultsEndoAll/Counts_endoOnly.csv')
